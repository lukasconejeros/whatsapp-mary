    ...
    <div className="flex h-screen overflow-hidden" style={{ background: '#FDF2F8' }}>
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <header className="flex items-center gap-3 shrink-0" style={{ height: 48, padding: '0 20px', background: '#FFFFFF', borderBottom: '1px solid #FBCFE8' }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: '#831843' }}>Finanzas</span>
          <div className="flex items-center gap-1" style={{ marginLeft: 8 }}>
            <button onClick={() => setMes(m => shiftMonth(m, -1))} style={{ display: 'flex', border: 'none', background: 'transparent', cursor: 'pointer', color: '#DB2777' }}><ChevronLeft size={18} /></button>
            <span style={{ fontSize: 13, fontWeight: 600, color: '#831843', minWidth: 130, textAlign: 'center' }}>{monthLabel(mes)}</span>
            <button onClick={() => setMes(m => shiftMonth(m, 1))} style={{ display: 'flex', border: 'none', background: 'transparent', cursor: 'pointer', color: '#DB2777' }}><ChevronRight size={18} /></button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto" style={{ padding: '18px 20px' }}>
          ...
          <div className="flex gap-3" style={{ marginBottom: 18, flexWrap: 'wrap' }}>
            {[
              ...
              { l: 'Costos', v: totalCos, c: '#BE185D', bg: '#FCE7F3', bd: '#FBCFE8' },
              { l: 'Ganancia neta', v: ganancia, c: ganancia >= 0 ? '#15803D' : '#DC2626', bg: '#FFFFFF', bd: '#FBCFE8' },
            ].map(card => (
              <div key={card.l} style={{ flex: 1, minWidth: 160, background: card.bg, border: `1px solid ${card.bd}`, borderRadius: 14, padding: '14px 16px' }}>
                <p style={{ fontSize: 12, color: '#9D5577', marginBottom: 6 }}>{card.l}</p>
            ...
            : grupos.map(([tipo, g]) => (
              <div key={tipo} style={{ background: '#fff', border: '1px solid #FBCFE8', borderRadius: 12, marginBottom: 8, overflow: 'hidden' }}>
                <button onClick={() => setOpenTipo(p => p === tipo ? null : tipo)}
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#831843', flex: 1 }}>{tipo}</span>
                  <span style={{ fontSize: 11, color: '#B57795' }}>{g.items.length} mov.</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#BE185D', minWidth: 110, textAlign: 'right' }}>{formatCLP(g.total)}</span>
                </button>
                {openTipo === tipo && (
                  <div style={{ borderTop: '1px solid #FCE7F3' }}>
                    {g.items.map(r => (
                      <div key={r.id} className="flex items-center gap-3" style={{ padding: '8px 16px', borderBottom: '1px solid #FDF2F8', fontSize: 12 }}>
                        <span style={{ color: '#9D5577', width: 78 }}>{r.fecha}</span>
                        <span style={{ flex: 1, color: '#6B5563' }}>{r.detalle || r.apoderado || r.notas || '—'}</span>
                        <span style={{ color: '#831843', fontWeight: 600 }}>{formatCLP(amount(r))}</span>
                        <button onClick={() => openEdit(r)} title="Editar" style={{ display: 'flex', border: 'none', background: 'transparent', cursor: 'pointer', color: '#D98BB0' }}><Pencil size={14} /></button>
                        <button onClick={() => del(r)} title="Borrar" style={{ display: 'flex', border: 'none', background: 'transparent', cursor: 'pointer', color: '#D98BB0' }}><Trash2 size={14} /></button>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div onClick={closeForm} style={{ position: 'fixed', inset: 0, background: 'rgba(131,24,67,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <form onClick={e => e.stopPropagation()} onSubmit={submitForm}
            ...
            <div className="flex items-center justify-between" style={{ marginBottom: 14 }}>
              <p style={{ fontSize: 15, fontWeight: 700, color: '#831843' }}>{editId ? 'Editar' : 'Agregar'} {tab === 'ganancias' ? 'ingreso' : 'costo'}</p>
            ...
            <label style={{ fontSize: 12, color: '#9D5577' }}>Fecha</label>
            <input type="date" value={form.fecha} onChange={e => setForm({ ...form, fecha: e.target.value })}
              style={{ width: '100%', margin: '4px 0 12px', padding: '8px 10px', borderRadius: 8, border: '1px solid #FBCFE8', fontFamily: 'inherit', fontSize: 13 }} />
            <label style={{ fontSize: 12, color: '#9D5577' }}>Tipo</label>
            <select value={form.tipo} onChange={e => setForm({ ...form, tipo: e.target.value })}
              style={{ width: '100%', margin: '4px 0 12px', padding: '8px 10px', borderRadius: 8, border: '1px solid #FBCFE8', fontFamily: 'inherit', fontSize: 13, background: '#fff' }}>
              <option value="">— elegir —</option>
              {tipos.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
            <label style={{ fontSize: 12, color: '#9D5577' }}>Monto (CLP)</label>
            <input inputMode="numeric" value={form.monto} onChange={e => setForm({ ...form, monto: e.target.value })} placeholder="0"
              style={{ width: '100%', margin: '4px 0 12px', padding: '8px 10px', borderRadius: 8, border: '1px solid #FBCFE8', fontFamily: 'inherit', fontSize: 13 }} />
            <label style={{ fontSize: 12, color: '#9D5577' }}>Detalle</label>
            <input value={form.detalle} onChange={e => setForm({ ...form, detalle: e.target.value })} placeholder="opcional"
              style={{ width: '100%', margin: '4px 0 16px', padding: '8px 10px', borderRadius: 8, border: '1px solid #FBCFE8', fontFamily: 'inherit', fontSize: 13 }} />
