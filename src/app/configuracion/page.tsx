    ...
    <div className="flex h-screen overflow-hidden" style={{ background: '#EFF6FF' }}>
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <header className="flex items-center shrink-0"
          style={{ height: 56, padding: '0 28px', background: '#fff', borderBottom: '1px solid #BFDBFE' }}>
          <h1 style={{ fontSize: 15, fontWeight: 700, color: '#1E3A5F', letterSpacing: '-0.01em' }}>Entrenar IA</h1>
          <div className="flex-1" />
          <button onClick={save} disabled={saving}
            className="flex items-center gap-1.5"
            style={{ height: 34, padding: '0 16px', borderRadius: 8, border: 'none',
              background: saved ? '#16A34A' : '#2563EB', color: '#fff', fontSize: 13, fontWeight: 600,
        ...
        <div className="flex-1 overflow-y-auto" style={{ padding: '28px' }}>
          <div style={{ maxWidth: 720, margin: '0 auto' }}>
            ...
            {/* Intro */}
            <div style={{ display: 'flex', gap: 12, padding: '14px 16px', borderRadius: 12, background: '#DBEAFE', border: '1px solid #93C5FD', marginBottom: 24 }}>
              <Sparkles size={18} style={{ color: '#2563EB', flexShrink: 0, marginTop: 1 }} />
              <p style={{ fontSize: 13, color: '#1E3A5F', lineHeight: 1.55 }}>
...
function SeccionEditor({ titulo, contenido, onChange, tecnica }: { titulo: string; contenido: string; onChange: (v: string) => void; tecnica?: boolean }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, border: `1px solid ${tecnica ? '#FDE68A' : '#BFDBFE'}`, overflow: 'hidden', boxShadow: '0 1px 2px rgba(30,58,95,0.04)' }}>
      <div style={{ padding: '10px 16px', borderBottom: '1px solid #EFF6FF', background: tecnica ? '#FFFBEB' : '#F8FAFF' }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: '#1E3A5F' }}>{titulo}</span>
      </div>
      <textarea
        value={contenido}
