...
export default function AppNav() {
  ...
  return (
    <nav className="app-sidebar flex flex-col h-full shrink-0"
      style={{ width: 212, background: '#FFFFFF', borderRight: '1px solid #EBDCE3' }}>

      {/* Marca */}
      <div className="app-brand flex items-center gap-3" style={{ padding: '20px 18px 16px', borderBottom: '1px solid #F3E7EC' }}>
        <div className="flex items-center justify-center shrink-0"
          style={{ width: 38, height: 38, borderRadius: 11, background: '#fff', border: '1px solid #EBDCE3', boxShadow: '0 4px 12px rgba(236,72,153,0.2)' }}>
          <Brush size={20} strokeWidth={2} style={{ color: '#B76E79' }} />
        </div>
        <div>
          <p style={{ fontSize: 15, fontWeight: 700, color: '#4A2E39', lineHeight: 1.1, letterSpacing: '-0.01em' }}>Arteluk</p>
          <p style={{ fontSize: 11, color: '#9A8188', marginTop: 3, letterSpacing: '0.01em' }}>Panel de Mary</p>
        </div>
      </div>

      {/* Navegación */}
      <p className="app-menuLabel" style={{ fontSize: 10, fontWeight: 700, color: '#B6A2AA', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '16px 20px 6px' }}>Menú</p>
          ...
          return (
            <Link key={href} href={href}
              ...
              style={{ padding: '8px 12px', fontSize: 13, fontWeight: active ? 600 : 450, borderRadius: 10,
                color: active ? '#8E5563' : '#6B5563',
                background: active ? '#F3E7EC' : 'transparent',
                boxShadow: active ? 'inset 0 0 0 1px #EBDCE3' : 'none',
                textDecoration: 'none' }}
              onMouseEnter={e => { if (!active) (e.currentTarget as HTMLElement).style.background = '#FBF7F9' }}
              onMouseLeave={e => { if (!active) (e.currentTarget as HTMLElement).style.background = 'transparent' }}>
              <Icon size={15} strokeWidth={active ? 2.2 : 1.7} style={{ color: active ? '#B76E79' : '#B08097' }} />
      ...
      <div className="app-footer" style={{ padding: '0 12px 16px' }}>
        <div style={{ padding: '10px 12px', borderRadius: 11, background: '#FBF7F9', border: '1px solid #F3E7EC' }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: '#8E5563', marginBottom: 2 }}>Arteluk · academia de arte</p>
          <p style={{ fontSize: 11, color: '#9A8188' }}>Hecho con cariño 💕</p>
        </div>
      </div>
    </nav>
  )
